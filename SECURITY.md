...
8. **Formal models cover the handshake and asynchronous initiation, not everything.** Three Tamarin theories are machine-checked with Tamarin 1.12.0 (formal/RESULTS.md): all 12 handshake lemmas and all 9 asynchronous-initiation lemmas (including post-quantum forward secrecy through one-time prekeys) verify; of the collision-rule theory the executable trace is found and its three consistency lemmas are undecided within a five-minute budget (no counterexample). The ratchet, group sender keys and mesh privacy are tested but not modelled. Writing and checking the models surfaced four protocol fixes now implemented: binding the responder's signature to the initiator's hello, a responder-side nonce cache, rejecting stale initiation blocks from a lost collision (the loser announces the ephemeral it abandoned, the winner blacklists every init it ignored), and remembering every accepted initiation ephemeral so a replayed older initiation cannot rebuild a stale session.
9. **No external audit.**
